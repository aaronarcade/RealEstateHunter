...
/** Keep US listings when --us-only is set (all US states, not FL-only). */
function isUsListing(listing) {
  const state = String(listing.state || '').toUpperCase();
  const country = listing.country == null ? 'US' : listing.country;
  return country === 'US' && US_STATES.has(state);
}
